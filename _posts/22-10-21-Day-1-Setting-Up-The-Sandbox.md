...
At first, it's just a TON of sysadmin stuff ... a good opportunity to revisit lots of installation and understanding some of the basic skills ... a first, we will just want to look at obtaining an affordable learning sandbox.  This involves a very, very, very basic awareness of what we might want to look at in performance monitoring and tracing as we get into actually doing any analysis.  We will want to push the system and we will want an understanding of how the computation is happening or where things are getting stuck -- there's a LOT going on behind the scenes or under the hood beyond just the Jupyter notebook processing indicators ... most of these things are things one does not need to master UNTIL the time comes, but it is necessary at the start to have a modicum of awareness of what is happening computewise.
